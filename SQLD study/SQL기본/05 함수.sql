...
SELECT -- 데이터 변환의 종류
     ...
     , TO_CHAR(10.25, '$999,999,999.99') AS "TO_CHAR(10.25, '$999,999,999.99')" -- 달러 형식으로 변경
     , TO_CHAR(12500, 'L999,999,999') AS "TO_CHAR(12500, '$999,999,999.99')" -- 원화 형식으로 변경
     , TO_NUMBER('100') + TO_NUMBER('100') AS "TO_CHAR(10.25, '$999,999,999.99')" -- 문자를 숫자로 변경
  FROM DUAL
; 

-----------------------------------------------------
-- 단일 행 CASE, DECODE
SELECT CASE WHEN SAL_CD = '100001' THEN '기본급' -- CASE
...
SELECT DECODE(SAL_CD, '100001', '기본급', '100002', '상여급', '기타')  AS SAL_NM -- DECODE: SAL_CD의 값이 '100001'이면 기본급을 가져오고 '100002'이면 상여급을 가져오고 그렇지 않으면 디폴트 값인 '기타'를 가져온다.
  FROM TB_SAL 
;  
...
SELECT NVL(UPPER_DEPT_CD , '최상위부서') AS 상위부서 -- UPPER_DEPT_CD가 NULL이면 '최상위부서'로 출력
  FROM TB_DEPT
...
SELECT NVL(MAX(EMP_NM), '존재안함') AS EMP_NM -- MAX 함수를 사용하면 '존재안함'(한개의 ROW)으로 출력된다. (시험에 자주 출제된다.) 
FROM TB_EMP
WHERE EMP_NM = '박찬호'; 

SELECT NVL(EMP_NM, '존재안함') AS EMP_NM -- MAX 함수를 사용하지 않으면 공집합으로 출력된다. (시험에 자주 출제된다.)
FROM TB_EMP
WHERE EMP_NM = '박찬호'; 

SELECT NULLIF('박찬호', '박지성') -- 같지않으면 박찬호(앞의 값) 리턴, 같으면 NULL 리턴
FROM DUAL
;

SELECT NVL(NULLIF('박찬호', '박찬호'), '같으면널') -- 같지않으면 박찬호(앞의 값) 리턴, 같으면 '같으면널' 리턴
FROM DUAL
;


SELECT COALESCE(NULL, NULL, 0) AS SAL -- NULL이 아닌 첫번째 값을 리턴한다.(0)
FROM DUAL;

SELECT COALESCE(5000, NULL, 0) AS SAL -- NULL이 아닌 첫번째 값을 리턴한다.(5000)
FROM DUAL;

SELECT COALESCE(NULL, 6000, 0) AS SAL -- NULL이 아닌 첫번째 값을 리턴한다.(6000)
FROM DUAL;